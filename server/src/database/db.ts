...
// Web push subscription row — one row per (browser/device, account) pair.
// A single device can hold a row for each account signed in on it, so
// switching the active account there doesn't steal the subscription away
// from the account it was created for.
export type PushSubscription = {
  id: Generated<number>; // serial PK, auto-generated by the DB on insert
  did: string; // owner DID
  endpoint: string; // push service endpoint URL (unique per did + endpoint pair)
...
migrations["008"] = {
  async up(db: Kysely<any>) {
    ...
    // re-enable notifications, which is a one-tap fix.
