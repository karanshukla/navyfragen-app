...
describe("MessageController", () => {
  ...
  describe("addExampleMessages", () => {
    ...
    test("returns 500 when service throws", async () => {
      ...
      const logger = { info: mock(), error: mock(), warn: mock(), debug: mock() };
  ...
  describe("respondToMessage", () => {
    ...
    test("passes includeQuestionAsImage=true to service when present in body", async () => {
      ...
      assert.deepStrictEqual(res.json.mock.calls[0][0], { success: true });
      const callArgs = (svc.respondToMessage as any).mock.calls[0];
    ...
    test("returns 500 with error message when service throws with non-empty message", async () => {
      ...
      assert.strictEqual(res.status.mock.calls[0][0], 500);
      assert.strictEqual(res.json.mock.calls[0][0].error, "Failed to post to Bluesky");
    });
  ...
  describe("sendMessage", () => {
    ...
    test("returns result on success", async () => {
      ...
      assert.deepStrictEqual(res.json.mock.calls[0][0], { tid: "t2" });
    });
    ...
    test("logs an error when the fire-and-forget push notification rejects", async () => {
      ...
      assert.strictEqual(logger.error.mock.calls[0][1], "Failed to send push notification");
    });
    ...
    test("returns 500 with fallback message when error has empty message string", async () => {
      ...
      await controller.sendMessage(makeReq({ body: { recipient: "did:foo", message: "hi" } }), res);
      assert.strictEqual(res.status.mock.calls[0][0], 500);
      assert.strictEqual(res.json.mock.calls[0][0].error, "Failed to send message");
    });
  });
  ...
  describe("getMessages", () => {
    ...
    test("returns messages on success", async () => {
      const svc = makeService({ getMessages: mock(async () => [{ tid: "t1" }]) });
      const logger = { info: mock(), error: mock(), warn: mock(), debug: mock() };
      const controller = new MessageController(svc, logger, makeCtx());
      const res = makeRes();
      await controller.getMessages(makeReq(), res);
      assert.deepStrictEqual(res.json.mock.calls[0][0], { messages: [{ tid: "t1" }] });
    });
  ...
  describe("deleteMessage", () => {
    ...
    test("returns 500 with fallback message when error has empty message string", async () => {
      ...
      await controller.deleteMessage(makeReq({ params: { tid: "t1" } }), res);
      assert.strictEqual(res.status.mock.calls[0][0], 500);
      assert.strictEqual(res.json.mock.calls[0][0].error, "Failed to delete message");
    });
  });
  ...
  describe("deleteAccount", () => {
    ...
    test("logs an error when dropping push subscriptions (fire-and-forget) rejects", async () => {
      ...
      await new Promise((resolve) => setImmediate(resolve));
      assert.strictEqual(logger.error.mock.calls.length, 1);
      assert.strictEqual(logger.error.mock.calls[0][1], "Failed to delete push subscriptions");
    });
    ...
    test("returns 500 with fallback message when error has empty message string", async () => {
      ...
      await controller.deleteAccount(makeReq(), res);
      assert.strictEqual(res.status.mock.calls[0][0], 500);
      assert.strictEqual(res.json.mock.calls[0][0].error, "Failed to delete account data");
    });
  });
  ...
  describe("syncMessages", () => {
    ...
    test("returns sync result on success", async () => {
      const svc = makeService({ syncMessages: mock(async () => ({ synced: 5 })) });
      const ctx = makeCtx({ pdsSyncEnabled: true });
      const logger = { info: mock(), error: mock(), warn: mock(), debug: mock() };
      const controller = new MessageController(svc, logger, ctx);
      const res = makeRes();
      await controller.syncMessages(makeReq(), res);
      assert.deepStrictEqual(res.json.mock.calls[0][0], { synced: 5 });
    });
