...
1. **Brand primitives** (`--nf-royal`, `--nf-grad-mark`, …) — scheme-independent raw
   palette. Referenced only by the semantic layer in the same file.
2. **Semantic tokens** (`--nf-surface`, `--nf-link`, `--nf-nav-active-bg`, …) — named
...
3. **On-gradient tokens** (`--nf-on-grad`, `--nf-on-grad-muted`,
   ...
   gradients are dark in both schemes. `--nf-on-grad-faint` is for rules and progress
...
- `--nf-grad-mark` — the primary brand gradient; use it for every interactive card
  background (login, ask, inbox hero, gradient question cards)
- `--nf-grad-dark` — reserved for the "default" image-export theme preview; not a UI
  surface
...
come from `--nf-accent-text` / `--nf-link` rather than fighting the provider's
`--mantine-color-*-text`.
...
tint. It also fails on a declared `--nf-*` token nothing references and on a
