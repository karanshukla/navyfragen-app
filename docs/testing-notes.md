...
1. **undici_v8 module-load crash** — undici 8.x's `CacheStorage` constructor throws `webidl.util.markAsUncloneable is not a function` under Bun 1.3.x at module-load (the static import evaluates undici's top-level code before any version-check runs). Not fixed in any undici 8.x (verified through 8.9.0); not fixable by upgrading Bun (already latest) or `@atproto-labs/fetch-node` (already latest 0.3.7 — verified against upstream `bluesky-social/atproto` at HEAD, which still eagerly imports `undici_v8` in `unicast.ts` with no Bun-specific handling).
