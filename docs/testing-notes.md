...
### `server/src/tests/mock-shim.ts` — whole-file `/* v8 ignore start/stop */`

**Why ignored:** This is the only non-`.test.ts` module under `server/src/tests/`, and the server's `c8.exclude` does not exclude `src/tests/**` (see the note above on the per-file rollout), so it is measured like production source. It is pure test infrastructure that reimplements node:test's `mock` surface, and parts of that surface exist to match the API rather than because the suite calls them (`mock.timers`'s unimplemented members, `mockReturnValue`, the `instances` array). Covering those would mean writing tests for a test helper's unused branches. `/* v8 ignore start */` after the header comment and `/* v8 ignore stop */` at the end of the file exclude it wholesale.

This is a third accepted use of the marker beyond the two listed in `CLAUDE.md` (`catch {}` around non-throwing DOM work, and TypeScript-narrowed unreachable branches): a test-infrastructure module that `c8.exclude` would otherwise have to list. Adding `src/tests/**` to `c8.exclude` was considered and rejected — it would also drop every `.test.ts` file from the report, and those currently sit at 100% and hold the aggregate above the 97% Coveralls threshold.
...
- **`--isolate`** — Bun's `mock.module` is process-global and **not restorable** (`clearAllMocks` clears `mock.fn` call history but does not unmock modules), whereas node:test scopes module mocks per file. `--isolate` gives each test file a fresh module registry so a file's `mock.module` (e.g. `auth-service.test.ts` mocking `session-agent`) can't leak into other files that import the real module (e.g. `session-agent.test.ts`, which tests `initializeAgentFromSession` for real). Treat it as defence in depth, not a licence to write partial module mocks — see the `auth-service.test.ts` note above.
...
### Bun version floor (`src/tests/assert-bun-version.js`)

`bun test` accepts unknown flags **without erroring**. `--isolate` landed in Bun 1.3.14; on 1.3.11 the flag is dropped silently and the suite reports 4 files dead on a missing-export `SyntaxError` with no hint that a flag was ignored. `test:bun` and `test:bun:watch` therefore run `src/tests/assert-bun-version.js` first, which fails with the actual reason if `Bun.version` is below the floor. CI still installs `bun-version: latest` so the canary keeps tracking Bun; the guard is what makes an incompatible local Bun legible.
