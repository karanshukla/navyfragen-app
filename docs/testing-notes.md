...
The block/pass matrix below was re-run against the re-cut patch on Bun 1.4 canary. Literal loopback, private and link-local addresses are rejected as non-unicast, a DNS name resolving to loopback is rejected as "not a public domain", `file:` is rejected as an unsupported protocol, a custom dispatcher is rejected outright, and a public hostname reaches the wrapped fetch. Re-run it on any re-cut.
