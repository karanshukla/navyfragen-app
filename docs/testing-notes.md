...
## `html-to-image/app.js` (Node `--experimental-test-coverage`)

CI (`html-to-image-tests` in `.github/workflows/Tests.yml`) runs `node --test app.test.js` with no coverage flag and no threshold gate — coverage here is a local diagnostic, not an enforced bar, unlike the client's 100%-on-all-metrics gate or the server's 97%-lines gate.

**Entry-point bootstrap block excluded via `node:coverage disable`/`enable`:** the `if (process.argv[1] === fileURLToPath(import.meta.url)) { ... }` block at the bottom of `app.js` (server listen + `SIGINT`/`SIGTERM` handlers) only runs when the file is executed directly, never when `app.test.js` imports it — the same rationale as excluding `server/src/index.ts` on the server side. Node's built-in test coverage reporter (unlike Bun's) honors `/* node:coverage disable */` / `/* node:coverage enable */` comments for line-level reporting. Note this only suppresses the **line** metric for the region — Node's function/branch percentages still count the bootstrap-only closures (`launch`, `log`, `shutdown`) inside it as uncovered, which is why `app.js`'s branch/func percentages don't reach a clean 100 even though its line coverage does. Since neither is CI-gated for this service, that display quirk is left as-is rather than chased.

**`waitForVisualReadiness`'s in-page callbacks (`page.evaluate`'s fonts-ready check, `page.waitForFunction`'s image-completeness predicate) are exercised by calling the captured function directly** in the relevant tests (`"evaluate callback waits on document.fonts.ready when present"`, `"waitForFunction predicate checks every image is complete..."`), with a fake `document` global standing in for the real page's — these functions are serialized by Puppeteer and actually execute in the browser page context, not in this Node process, so the mock `page.evaluate`/`page.waitForFunction` in `app.test.js` only *record* the function reference; a test has to invoke it explicitly to get real coverage and verify its logic.
