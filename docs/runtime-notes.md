...
- `bun run test` / `test:coverage` add `--no-env-file` (Bun otherwise auto-loads
  ...
  because no 1.4 types are published yet; move it when they are.
...
- **Never call `dns.setServers()` unconditionally.** Under Node it only rebinds the
  ...
  container DNS included. Bun 1.4 matches Node (verified against a container DNS name
  on `oven/bun:canary` vs `oven/bun:1.3`), so that specific hazard is gone. The
