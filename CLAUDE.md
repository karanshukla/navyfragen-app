...
- **Bind `env.HOST`, not a hardcoded `"::"`.** On an IPv6-less container network Bun fails a `"::"` bind with a spurious `EADDRINUSE` (`errno: 0`) instead of falling back the way Node does. `HOST` is required in production — set it to `"::"` where dual-stack ingress is needed.
